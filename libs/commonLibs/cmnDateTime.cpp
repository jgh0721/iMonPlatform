...
namespace nsCommon
{
    namespace nsCmnDateTime
    {
        ...
        CString formatElapsedTime( unsigned int elapsedTimeSec, const CString& fmt /*= _T("%H 시간 %M 분 %S 초") */ )
        {
            ...
            CString formatText = fmt;
            if( formatText.Find( _T("%H") ) >= 0 )
            {
                formatText.Replace( _T("%H"), format( _T("%d"), hour ).c_str() );
                formatText.Replace( _T("%M"), format( _T("%d"), minute ).c_str() );
            }
            else
            {
                formatText.Replace( _T("%M"), format( _T("%d"), minute + ( hour * 60 ) ).c_str() );
            }

            formatText.Replace( _T("%S"), format( _T("%d"), sec ).c_str() );
